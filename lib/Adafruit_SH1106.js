...
const SH1106_INIT_SEQ_2 = [
    2,                                     // 2 commands
    SH1106_SET_DISPLAY_OFFSET,     0x01,   // Set Display Offset
    0x00,                                  // sets offset pro to 0
    SH1106_SET_START_LINE | 0x00,  0x00    // Line #0 - No Args, no delay.
];
...
class Adafruit_SH1106 extends Display_Base {
    ...
    */
    ...
    begin() {
        ...
        self._executeInitSequence(SH1106_INIT_SEQ_4);
    ...
    */
    /**************************************************************************/
    display() {
        const self = this, colOffset = self._colOffset, buffer = self._buffer;
        const pageEnd = (self.HEIGHT / 8) & 0xFF;
        const bytesPerPage = self.WIDTH;
        const col = 0x00 + colOffset;

        // As the SH1106 supports internal ram of 132x64, we cannot simply write our internal buffer as it is 128x64, 128x32 or 96x64.
        // Therefore, we must set the starting address and write a single page at a time.
        // Sure, we could calculate and write 1 big buffer, but this way is much easier.
        
        for (let page = 0x00, index = 0x00; page < pageEnd; page++,index+=bytesPerPage) {
            self._SH1106_command1(SH1106_PAGE_ADDR | page);
            ...
            const pageData = (typeof(Buffer.prototype.subarray) !== "undefined") ? buffer.subarray(index, index + bytesPerPage) : buffer.slize(index, index + bytesPerPage);
            self._SH1106_data(pageData);
        }
        return self;
    }
